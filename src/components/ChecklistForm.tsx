...
const ChecklistForm = () => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Simulate high-tech fetch progress
  ...
  if (status === 'fetching') {
    return (
      <div className="p-8 sm:p-12 font-mono text-left">
        ...
        <div className="text-slate-500 text-[10px] space-y-1 uppercase tracking-widest text-left">
          ...
          {progress > 50 && <div>{">"} Decrypting payload...</div>}
          {progress > 80 && <div>{">"} Bypassing crawler blocks...</div>}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-8 sm:p-12 text-left">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div>
          <label htmlFor="check-email" className="block text-[10px] font-black text-neon-cyan uppercase tracking-[0.3em] mb-4">_Comm_Address</label>
          <input
            type="email"
            id="check-email"
            name="email"
